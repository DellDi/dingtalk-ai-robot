...
# 导入最新的AutoGen v0.4 AgentChat API
from autogen_agentchat.agent import Agent, AgentSystem, AssistantAgent, UserProxyAgent
from autogen_agentchat.openai import ChatCompletion
from autogen_agentchat.messages import ModelMessage, UserMessage, SystemMessage
    ...
    def _setup_api_keys(self):
        ...
        if settings.AZURE_OPENAI_API_KEY:
            ...
            if settings.AZURE_DEPLOYMENT_NAME:
                os.environ["AZURE_OPENAI_DEPLOYMENT"] = settings.AZURE_DEPLOYMENT_NAME
            self.is_azure = True
        # 否则使用OpenAI
...
""",
    ...
    async def process_message(self, text: str, sender_id: str, conversation_id: str) -> Optional[str]:
        """
        ...
        try:
            ...
            if "知识" in text or "查询" in text or "资料" in text:
                ...
                # 使用知识库专家智能体
                chat_res = await self.user_proxy.initiate_chat(
                    recipient=self.knowledge_agent,
                ...
                # 使用JIRA专家智能体
                chat_res = await self.user_proxy.initiate_chat(
                    recipient=self.jira_agent,
                    message=text,
                )
                
                return self._extract_last_response(chat_res)
                
            elif "服务器" in text or "升级" in text or "dify" in text.lower() or "ssh" in text.lower():
                # 使用服务器专家智能体
                chat_res = await self.user_proxy.initiate_chat(
                    recipient=self.server_agent,
                    message=text,
                )
